...
def train(model, dataloaders, loss_fn, optimizer, scheduler, iter_per_epoch, num_epochs = 10):  
    ...
    if load_from_file:
        ...
        for _ in range(saved_epochs*iter_per_epoch):
            scheduler.step()
    else:
        ...
        for phase in ["train", "val"]:
            ...
            if phase == 'val':
                with open(os.path.join(train_path, "hyp.txt"), 'w') as f:
                    f.write(epoch+1) # +1 because of new epoch - current is done 
                    f.close()
                correct_top1 = correct_top1.item() / processed_data
                correct_top5 = correct_top5.item() / processed_data
                torch.save(model.state_dict(), os.path.join(train_path, "last.pt"))
                if correct_top1 > best_acc:
                    best_acc = correct_top1
...
if __name__ == '__main__':
    ...
    parser.add_argument("-id", "--restore_run_id", required=False, help="run id to restore")
    args, unknown = parser.parse_known_args()

    saved_model = args.restore_run_id
